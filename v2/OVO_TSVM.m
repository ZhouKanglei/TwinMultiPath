...
if ~exist([resFolder, '/mainExp/OVO_TSVM'], 'dir')
    mkdir([resFolder, '/mainExp/OVO_TSVM']);
end
...
for dataSetNum = dataRange
    for times = 1 : 10
        ...
        for clssNum_i = 1 : classNum
            for clssNum_j = clssNum_i + 1 : classNum
                cntNum = cntNum + 1;
                y(trainData(:, 1) == clssNum_i) = 1;
                y(trainData(:, 1) == clssNum_j) = -1;
                A = X((y == 1), :);
        		B = X((y == -1), :);
                [u, v] = TSVM(A, B, lambda_one, lambda_two);
                TSVMModels{cntNum, 1} = u;
                TSVMModels{cntNum, 2} = v;
            end
        end
        time(times) = toc;
        ...
        for clssNum_i = 1 : classNum
            for clssNum_j = clssNum_i + 1 : classNum
                cntNum = cntNum + 1;
                score = predictTSVM(TSVMModels{cntNum, 1},...
                    TSVMModels{cntNum, 2}, testX);
                Scores(:, clssNum_i) =  score(:, 1); 
                Scores(:, clssNum_j) =  score(:, 2); 
            end
        end
    ...
    %% Save data.
    %% Save data.
    dataFullPath = [resFolder, '/mainExp/OVO_TSVM/',...
