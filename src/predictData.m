function [corrRatio, y_pred] = predictData(...
    data, lambda_1, alpha, beta, lambda_2, mu, rho)
    ...
    if distOpt == 1
        ...
        y_pred((y_pred(:, 1) == 1 & y_pred(:, 2) == 1), 1) = -1;
        y_pred((y_pred(:, 1) == 1 & y_pred(:, 2) == 1), 2) = -1;
    ...
    if distOpt == 2
        ...
        y_pred((y_pred(:, 1) == y_pred(:, 2)), 1) = -1;
        y_pred((y_pred(:, 1) == y_pred(:, 2)), 2) = -1;
...
%         y_pred((pos' == 1), 1) = ...
%             numTrainingSum / size(F, 1);
%         y_pred((pos' == 4), 2) =  ...
%             numTrainingSum / size(G, 1);
